...
export default function CardAcara({ data }) {
    console.log("🚀 ~ CardAcara ~ data:", data)

    const [acaras, setAcara] = useState(data);
    const [cari, setCari] = useState('');
    const [hasilCari, setHasilCari] = useState([]);
    const router = useRouter()
    // console.log("🚀 ~ CardAcara ~ hasilCari:", hasilCari)

    // console.log("🚀 ~ CardAcara ~ cari:", cari)
    console.log(acaras);



    const getAcara = async (cari) => {
        if (cari.length > 0) { // Opsional: Mulai pencarian setelah 2 karakter
            const res = await fetch(`/api/seller/acara/cari_acara/${cari}`, {
                method: 'GET'
            })
